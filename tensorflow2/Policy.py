...
class PPOPolicy:
    def __init__(self, pi_network, v_network, optimizer, clip_range, value_coeff, obs_dim, action_dim, initial_std=1.0) -> None:
        (
            self.pi_network,
            self.v_network,
            self.optimizer,
        ...
        self.log_std = tf.Variable(
            initial_value=tf.math.log(initial_std) * tf.ones(shape=self.action_dim),
            dtype=tf.float32,
            trainable=True
        )
        ...
        self.trainable_params = self.pi_network.trainable_variables + self.v_network.trainable_variables + [self.log_std]
    ...
    def update(self, obs_batch, action_batch, log_prob_batch, advantage_batch, return_batch):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:      
            ...
            value_loss = self.value_coeff * tf.reduce_mean(tf.square(values - return_batch))
